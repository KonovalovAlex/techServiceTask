INSERT INTO ROLE (ID, role) VALUES (1, 'ROLE_USER');
INSERT INTO ROLE (ID, role) VALUES (2, 'ROLE_ADMIN');
insert into product (ID, name, brand , price, quantity) VALUES (1,'cristian','dior','1000', 1);
insert into product (ID, name, brand , price, quantity) VALUES (2,'1cristian','1dior','1000', 2);
insert into product (ID, name, brand , price, quantity) VALUES (3,'2cristian','2dior','1000', 3);
insert into product (ID, name, brand , price, quantity) VALUES (4,'3cristian','3dior','1000', 7);
insert into product (ID, name, brand , price, quantity) VALUES (5,'4cristian','4dior','1000', 2);
